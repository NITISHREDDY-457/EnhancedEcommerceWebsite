...
from django.contrib import messages
from .forms import CustomSareeFeedbackForm
...
def cart_view(request):
    ...
    # Pass everything to the context for rendering
    context = {
        'products': products,
        'total': total,
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            image=sub.cleaned_data.get('image')
            
            if image:
                fs = FileSystemStorage()
                filename = fs.save(image.name, image)
                uploaded_file_url = fs.url(filename)
                message += f"\n\n[Image uploaded: {uploaded_file_url}]"
            send_mail(str(name)+' || '+str(email),message, settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVING_USER, fail_silently = False)
            return render(request, 'ecom/contactussuccess.html')
    return render(request, 'ecom/contactus.html', {'form':sub})


def custom_saree_feedback_view(request):
    form = CustomSareeFeedbackForm(request.POST or None)
    if form.is_valid():
        feedback = form.save(commit=False)
        if request.user.is_authenticated:
            feedback.user = request.user
        feedback.save()
        messages.success(request, "Thank you! Your customization request has been sent.")
        return redirect('custom-saree-feedback')  # use your url name

    return render(request, 'ecom/custom_saree_feedback.html', {'form': form})